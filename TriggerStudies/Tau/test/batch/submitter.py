...
parser.add_option("-c"  , "--cfg"     , dest = "cfg"      ,  help = "input cfg file"                                                  , default =  "../hlt.py"             )
parser.add_option("-d"  , "--outdir"  , dest = "outdir"   ,  help = "output dir on eos"                                               , default =  ""                      )
...
# os.system('cp ../pat.py {TIME}'.format(TIME=newFolder))
# os.system('cp ../online-tau-rereco_v3.2.py {TIME}'.format(TIME=newFolder))
# os.system('cp ../pat-and-out.py            {TIME}'.format(TIME=newFolder))
# os.system('cp ../pat-and-out_MC.py         {TIME}'.format(TIME=newFolder))
# os.system('cp ../source.py                 {TIME}'.format(TIME=newFolder))
# os.system('cp ../source_MC.py              {TIME}'.format(TIME=newFolder))
# os.system('cp ../uct2015.py                {TIME}'.format(TIME=newFolder))
# os.system('cp ../uct2015_MC.py             {TIME}'.format(TIME=newFolder))
...
#             if i=="": continue
#             print >> f1,'\t\t"'+ i.rstrip() + '",'
#       if 'patTuple.root' in line:
#           newline = line.replace('patTuple.root', '"{TIME}/patTuple.root"'.format(TIME=newFolder)).rstrip()
#           newline = line.replace('patTuple.root', '"{TIME}/{OUT}/patTuple_{NUM}.root"'.format(TIME=newFolder, OUT=options.outdir, NUM=str(k) )).rstrip()
#           newline = line.replace('outputFileNameSub', '"root://eoscms//eos/cms/store/user/fiorendi/{OUTDIR}/{TRIG}_{TAG}_{NUM}.root"'.format(OUTDIR=options.outdir, NUM=str(k), TAG=sample, TRIG=options.hlt )).rstrip()
  ...
  print >> f1, "process.patOut.fileName  = cms.untracked.string('{TIME}/patTuple_{NUM}.root')".format(TIME=newFolder, NUM=str(k))
  print >> f1, "process.source.fileNames = cms.untracked.vstring("
  for i in infiles[k*nfiles:(k+1)*nfiles]:
    if i=="": continue
    print >> f1,'\t\t"'+ i.rstrip() + '",'
  print >> f1, ")"
  ...
  for shline in sh:
      ...
      if 'RESFILE' in shline:
          myres  = shline.replace('RESFILE', '{TIME}/patTuple_{NUM}.root'.format(TIME=newFolder, OUT=options.outdir, NUM=str(k))).replace('EOSDIR', '{OUTDIR}/patTuple_{NUM}.root'.format(OUTDIR=options.outdir,NUM=str(k))).rstrip()
#           myres  = shline.replace('RESFILE', '{TIME}/{OUT}/patTuple_{NUM}.root'.format(TIME=newFolder, OUT=options.outdir, NUM=str(k))).replace('EOSDIR', '{OUTDIR}/patTuple_{NUM}.root'.format(OUTDIR=options.outdir,NUM=str(k))).rstrip()
#           myres  = shline.replace('RESFILE', '{TIME}/{OUT}/{TRIG}_{TAG}_{NUM}.root'.format(TIME=newFolder, OUT=options.outdir, NUM=str(k), TAG=sample, TRIG=options.hlt )).replace('EOSDIR', '/eos/cms/store/group/phys_muon/fiorendi/{OUTDIR}/{TRIG}_{TAG}_{NUM}.root'.format(OUTDIR=options.outdir,NUM=str(k), TAG=sample, TRIG=options.hlt)).rstrip()
...
#  os.system("bsub -u kjhaskdjhkaskjhdakjshda -q {QUEUE} {SH}".format(QUEUE=options.queue, SH=shName))

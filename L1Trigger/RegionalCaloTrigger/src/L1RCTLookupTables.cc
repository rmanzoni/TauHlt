				       ...
				       unsigned short twrNo) const
{
  ...
  if((ecalAfterMask == 0xFF &&
    ...
    {
      etIn7Bits = RCT_EG_PRECISION_MASK; // egamma path
    }
  if((ecalAfterMask == 0xFF &&
      rctParameters_->jetMETECalScaleFactors()[iAbsEta-1] != 0. )
     || (hcalAfterMask == 0xFF &&
  ...
  unsigned long shiftEtIn9Bits = etIn9Bits<<(RCT_EG_PRECISION+1);
  unsigned long shiftHE_FGBit = hOeFGVetoBit(ecal, hcal, fgbit)<<RCT_EG_PRECISION;
  ...
  if ( rctParameters_->jetMETECalScaleFactors()[iAbsEta-1] == 0.
    ...
    {
      // shift # of bits for EG, 9 bits for sum, 1 bit for HE_FG
      shiftActivityBit = activityBit(0., hcal,fgbit)<<(RCT_EG_PRECISION+9+1);
    }
  else if (rctParameters_->jetMETHCalScaleFactors()[iAbsEta-1] == 0. )
    {
      // shift # of bits for EG, 9 bits for sum, 1 bit for HE_FG
      shiftActivityBit = activityBit(ecal, 0.,fgbit)<<(RCT_EG_PRECISION+9+1);
    }
  else
    {
      // shift # of bits for EG, 9 bits for sum, 1 bit for HE_FG
      shiftActivityBit = activityBit(ecal, hcal,fgbit)<<(RCT_EG_PRECISION+9+1);
    }
...
unsigned int L1RCTLookupTables::eGammaETCode(float ecal, float hcal, int iAbsEta) const
{
  ...
  return convertToInteger(etLinear, rctParameters_->eGammaLSB(), RCT_EG_PRECISION);
}
